<?php
class Test extends MJ_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($productUrl)) {
            ...
            foreach ($productUrl as $url) {
                ...
                pr($url);
                var_dump(pq('.cpinfo1 .salesName').html());exit;
                $item['goods_name']           = pq('.cpinfo1 .title .salesName').next().text();
                $item['goods_sku']            = 'MCW_'.pq('#goods_id').val();
                $item['from_id']              = 4; //商品来源
                $item['brand_id']             = 0;
                $item['goods_weight']         = 100;
                $item['market_price']         = trim(pq('.cpjiage .shijia em').text(), '￥');
                $provide_price = pq('.cpjiage u.ajax_price').text();
                $item['shop_price']           = $this->rePrice($provide_price);
                $item['provide_price']        = $provide_price;
                $item['promote_start_date']   = 0;
                $item['promote_end_date']     = 0;
                $item['attr_set_id']          = 5;
                $item['goods_desc']           = pq('.brands_con .brands_middle').html();
                $item['wap_goods_desc']       = pq('.brands_con .brands_middle').html();
                $item['goods_note']           = $url;
                $item['attr_spec']            = '';
                $item['attr_value']           = '';
                $item['goods_img']            = '';
